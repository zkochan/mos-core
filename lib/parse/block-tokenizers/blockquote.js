  ...
  function tokenizeEach(index) {
    ...
    if (!prefixed) {
      if (parser.options.commonmark) {
        return tryBlockTokenize(parser, 'code', rest, true).then(function (found) {
          if (found) return index;

          return tryBlockTokenize(parser, 'fences', rest, true).then(function (found) {
            if (found) return index;

            return tryBlockTokenize(parser, 'heading', rest, true).then(function (found) {
              if (found) return index;

              return tryBlockTokenize(parser, 'lineHeading', rest, true).then(function (found) {
                if (found) return index;

                return tryBlockTokenize(parser, 'thematicBreak', rest, true).then(function (found) {
                  if (found) return index;

                  return tryBlockTokenize(parser, 'html', rest, true).then(function (found) {
                    if (found) return index;

                    return tryBlockTokenize(parser, 'list', rest, true).then(function (found) {
                      if (found) return index;
    ...
    function nextNotCommonmark() {
      if (!parser.options.commonmark) {
        return tryBlockTokenize(parser, 'definition', rest, true).then(function (found) {
          if (found) return index;

          return tryBlockTokenize(parser, 'footnoteDefinition', rest, true).then(function (found) {
            if (found) return index;
