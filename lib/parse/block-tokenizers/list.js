...
function tokenizeList(parser, value, silent) {
  ...
  function tokenizeEach(index) {
    ...
    if (currentMarker && RULE_MARKERS[currentMarker] === true) {
      return tryBlockTokenize(parser, 'thematicBreak', line, true).then(function (found) {
        if (found) {
          return;
        }
    ...
    function notCommonmarkNext() {
      if (!commonmark) {
        return tryBlockTokenize(parser, 'definition', line, true).then(function (found) {
          if (found) return;

          return tryBlockTokenize(parser, 'footnoteDefinition', line, true).then(function (found) {
            if (found) return;
    ...
    function notRuleMarker() {
      ...
      if (indented && item) {
        ...
        if (!pedantic) {
          return tryBlockTokenize(parser, 'fences', line, true).then(function (found) {
            if (found) return;
