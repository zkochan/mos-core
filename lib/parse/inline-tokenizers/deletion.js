...
function tokenizeDeletion(parser, value, silent) {
  ...
  if (!parser.options.gfm || value.charAt(0) !== '~' || value.charAt(1) !== '~' || isWhiteSpace(value.charAt(2))) {
    return;
  }
  ...
  while (++index < length) {
    ...
    if (character === '~' && previous === '~' && (!preceding || !isWhiteSpace(preceding))) {
      /* istanbul ignore if - never used (yet) */
      if (silent) {
        return true;
      }

      return parser.eat('~~' + subvalue + '~~')(parser.renderInline(nodeTypes.DELETE, subvalue, now));
    }
